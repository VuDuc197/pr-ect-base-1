...
export default class Vmo_handleToTalTickets extends LightningElement {
    ...
    async fetchCaseData() {
        await Promise.all(getToTalTickets(), getToTalThisMonth(), getToTalOpen(), getToTalProcessing(), getToTalClosed()) 
            .then(data => {
                this.totalTickets = getToTalTickets();
                this.totalThisMonth = getToTalThisMonth();
                this.totalOpen = getToTalOpen();
                this.totalProcessing = getToTalProcessing();
                this.totalClosed = getToTalClosed();
            })
            .catch((err) => {
